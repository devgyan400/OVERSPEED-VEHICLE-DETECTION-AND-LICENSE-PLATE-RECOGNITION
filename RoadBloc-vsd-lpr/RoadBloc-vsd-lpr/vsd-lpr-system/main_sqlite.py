...
def getLicenseNo(imgFile):
    ...
    #FIND ALL CONTOURS
    cnts = cv2.findContours(imgF, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)[1]
...
def storeData():
    #CONNECT TO DATABASE------------------------------------------------------------
    try:
        conn = sqlite3.connect('files/vsd-lpr.sqlite')
        cur = conn.cursor()
    ...
    while True:
        if len(licenseDataArr)>0:
            ...
            dataDict = {'date': date2, 'time': time2, 'speed': spd2, 'licNo': licNo2, 'licError': licError}
            cur.execute('CREATE TABLE IF NOT EXISTS OVERSPEED (date TEXT, time TEXT, speed NUMERIC, licNo TEXT, licError NUMERIC)')
            cur.execute('INSERT INTO OVERSPEED VALUES (?,?,?,?,?)',(date2, time2, spd2, licNo2, licError))
            conn.commit()
...
def sendSMS(data):
    ...
    try:
        #SQL QUERY TO RETRIEVE OWNER DETAILS OF NUMBER PLATE FROM A VEHICLEOWNER TABLE - Not Completed
        account_sid = ''
        auth_token = ''
        ...
        message = client.messages \
                        .create(
                             body="Your car : License Number : {}, broke the speed limit on {} at {}".format(data['licNo'], data['date'], time['time']),
