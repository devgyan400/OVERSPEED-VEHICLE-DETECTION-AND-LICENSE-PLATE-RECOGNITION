...
#CONNECT TO DATABASE------------------------------------------------------------
conn = sqlite3.connect('files/SDVR.sqlite')
cur = conn.cursor()
...
def saveCar(speed,image):
    ...
    cur.execute('CREATE TABLE IF NOT EXISTS SPEEDINGCAR (Date TEXT, EndTime TEXT, Speed_kmph NUMERIC, ImageLink TEXT)')
    cur.execute('INSERT INTO SPEEDINGCAR VALUES (?,?,?,?)',(curDate, curTime, speed, link))
    conn.commit()
